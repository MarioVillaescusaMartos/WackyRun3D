...
public class RoadDecoration : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (offset > laneLength * cellSize) 
        { 
            offset -= laneLength * cellSize; 
        ...
        if (shift)
        {
            ...
            for (int i = 0; i < lanes.Length; i ++)
            {
                for (int j = 0; j < lanes[i].segments.Length - 1; j++) 
                { 
                    lanes[i].segments[j] = lanes[i].segments[j + 1]; 
                }
    ...
    void RenderLanes()
    {
        for (int i = 0; i < lanes.Length; i++)
        {
            ...
            for (int j = 0; j < lanes[i].segments.Length; j++)
            {
                ...
                Graphics.DrawMesh(segmentMesh, segmentPosition, Quaternion.Euler(-90, 0, 0), segmentMaterial, gameObject.layer);             
            }
        }
    }
}
